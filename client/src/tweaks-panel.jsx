// Floating "Tweaks" panel — density, font pair and theme controls.
// A draggable glass card toggled from the sidebar.
...
export function useTweaks(defaults) {
  const [values, setValues] = React.useState(() => {
    try {
      const saved = JSON.parse(localStorage.getItem('switchyard.tweaks') || '{}')
  ...
  const setTweak = React.useCallback((key, val) => {
    setValues((prev) => {
      ...
      try { localStorage.setItem('switchyard.tweaks', JSON.stringify(next)) } catch (e) { /* ignore */ }
      return next
    })
  }, [])
...
// Segmented control. Falls back to a <select> when labels are too long to fit.
